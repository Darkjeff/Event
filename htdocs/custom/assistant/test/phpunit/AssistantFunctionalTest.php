...
/**
 * \file    test/functional/AssistantFunctionalTest.php
 * \ingroup assistant
 ...
 */
class AssistantFunctionalTest extends \PHPUnit_Extensions_Selenium2TestCase
{
...
//        )
	...
	public function testModuleEnabled()
	{
		...
		if (strstr($module_status_image->attribute('src'), 'switch_off.png')) {
			// Enable the module
			$this->byHref('modAssistant')->click();
		} else {
			// Disable the module
			$this->byHref('modAssistant')->click();
			// Reenable the module
	...
	{
		$this->url('/custom/assistant/admin/setup.php');
		$this->authenticate();
		return $this->assertContains('assistant/admin/setup.php', $this->url(), 'Configuration page');
	}
	...
	{
		$this->url('/custom/assistant/admin/about.php');
		$this->authenticate();
		return $this->assertContains('assistant/admin/about.php', $this->url(), 'About page');
	}
...
return $this->assertEquals(
	...
	public function testBoxDeclared()
	{
		...
		$this->authenticate();
		return $this->assertContains('assistantwidget1', $this->source(), "Box enabled");
	}
...
return $this->assertContains(
			'interface_99_modAssistant_AssistantTriggers.class.php',
...
return $this->assertContains(
			'tick.png',
			$this->byXPath('//td[text()="interface_99_modAssistant_MyTrigger.class.php"]/following::img')->attribute('src'),
			"Trigger enabled"
		);
	}
