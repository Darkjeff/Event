...
class Event extends CommonObject
{
     ...
     *  @return array          	list
     */
    function fetch_list($year = null)
    {
    	global $langs, $conf;
    	$sql = "SELECT rowid, label FROM ".MAIN_DB_PREFIX."event WHERE year(date_start) = $year AND fk_statut = 5;";	
    	$resql = $this->db->query($sql);
    	...
    	if ($resql)
    	{
    		...
    		while( $i < $num)
    		{
    			...
    			// $res[$i] = new stdClass();
    			// $res[$i]->id = $obj->rowid;
    			// $res[$i]->label = stripslashes($obj->label);
