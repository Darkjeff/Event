...
class Day extends Event
{
    ...
    function fetch_all($sortorder="DESC", $sortfield="t.date_event", $limit=0, $offset=1, $arch=0,$filter='')
    {
    	...
    	$sql = "SELECT";
    	$sql.= " t.rowid,";
    	$sql.= " (SELECT count(r.rowid) FROM ".MAIN_DB_PREFIX."event_registration as r WHERE r.fk_eventday = t.rowid AND r.fk_statut=0) as event_registration_0,";
    	$sql.= " (SELECT count(r.rowid) FROM ".MAIN_DB_PREFIX."event_registration as r WHERE r.fk_eventday = t.rowid AND r.fk_statut=1) as event_registration_1,";
    	$sql.= " (SELECT count(r.rowid) FROM ".MAIN_DB_PREFIX."event_registration as r WHERE r.fk_eventday = t.rowid AND r.fk_statut=8) as event_registration_8,";
    	$sql.= " (SELECT count(r.rowid) FROM ".MAIN_DB_PREFIX."event_registration as r WHERE r.fk_eventday = t.rowid AND r.fk_statut=4) as event_registration_4,";
    	$sql.= " (SELECT count(r.rowid) FROM ".MAIN_DB_PREFIX."event_registration as r WHERE r.fk_eventday = t.rowid AND r.fk_statut=5) as event_registration_5,";
		$sql.= " t.entity,";
		$sql.= " t.ref,";
		$sql.= " t.fk_soc,";
		$sql.= " t.fk_event,";
		$sql.= " ev.label as event,";
		$sql.= " t.datec,";
		$sql.= " t.tms,";
		$sql.= " t.date_event,";
		$sql.= " t.label,";
    	...
    	if ($resql)
    	{
    		...
    		while( $i < $num)
    		{
    			...
    			$this->line[$i]->event 	 			= $obj->event;
    			$this->line[$i]->event_registration_0	= $obj->event_registration_0;
    			$this->line[$i]->event_registration_1	= $obj->event_registration_1;
    			$this->line[$i]->event_registration_8	= $obj->event_registration_8;
    			$this->line[$i]->event_registration_4	= $obj->event_registration_4;
    			$this->line[$i]->event_registration_5	= $obj->event_registration_5;
				
				// Start add groupes	
				// $sql_igroupes = "SELECT el.* FROM ".MAIN_DB_PREFIX."event_level_day eld LEFT JOIN ".MAIN_DB_PREFIX."event_level el ON eld.fk_level = el.rowid WHERE eld.fk_eventday='". $obj->rowid ."' ;";
				$sql_igroupes = "SELECT s.* FROM ".MAIN_DB_PREFIX."event_level_day_cal as s WHERE s.fk_event_day = " . $obj->rowid;
				$resql3 = $this->db->query($sql_igroupes);
				$igroupes = array();
				if ($resql3) {
					$num3 = $this->db->num_rows($resql3);
					if ($num3) {
						$i3 = 0;
						// Affichage des dates
						while ($i3 < $num3)
