...
if ($action == 'setvar')
{
	...
	// developer.baalla@gmail.com   MAIN_LANDING_PAGE
	$MAIN_LANDING_PAGE = GETPOST('MAIN_LANDING_PAGE','alpha');
	// $res = dolibarr_set_const($db, 'MAIN_LANDING_PAGE', $event_public_active,'',0,'',$conf->entity);
...
$sql.= " WHERE entity IN (".$user->entity.",".$conf->entity.") AND name = 'MAIN_LANDING_PAGE'";
$result = $db->query($sql);
$obj = $db->fetch_object($result);

if ($MAIN_LANDING_PAGE) {
	// set value = /custom/event/public/
	if ( empty($obj) ) {
		$sql = "INSERT INTO ".MAIN_DB_PREFIX."const";
		$sql.= " (name, value, type, entity)";
		$sql.= " VALUES ('MAIN_LANDING_PAGE', 'custom/event/public/', 'chaine', ". $user->entity .")";
		$res = $db->query($sql);
		if (! $res > 0) $error++;
	} elseif ( !empty($obj) && !in_array($obj->value, array('custom/event/public/', 'custom/event/public', '/custom/event/public/', '/custom/event/public')) )
	{
		$sql = "UPDATE ".MAIN_DB_PREFIX."const";
		$sql.= " SET value = 'custom/event/public/'";
	...
	// DELETE
	if ( !empty($obj) && in_array($obj->value, array('custom/event/public/', 'custom/event/public', '/custom/event/public/', '/custom/event/public')))
	{
...
print '<tr>';
print '<td width="35%">Pour mettre en place la redirection vers la page publique (custom/event/public/) après la page de connexion</td>';
print '<td>';
$arrval=array('1'=>$langs->trans("Yes"), '0'=>$langs->trans("No"));
print $form->selectarray("MAIN_LANDING_PAGE",$arrval,strval($ichecked));
print '</td>';
print '</tr>';
// End MAIN_LANDING_PAGE ------------------------
