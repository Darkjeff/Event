...
if ($action == 'create' && $user->rights->event->write)
{
	...
	if (!empty(GETPOST("date_event"))){
		$date_event = date("Y-m-d", strtotime( str_replace('/', '-', GETPOST("date_event") ) ));
	}
         ...
         //////////////////////////////////////////////////////////////////			
		  ...
		  <?php
            ...
            if($conf->global->EVENT_HIDE_GROUP=='-1')
                ...
                print '<table class="noborder" style="max-width:500px;">';
                print '<tr class="liste_titre"><th colspan="2">' . $langs->trans("Statistics") . '</th></tr>';
                if (!empty($conf->use_javascript_ajax)) {
                    print '<tr><td align="left" colspan="2">';
					
					// Start Dev.Baalla@gmail.com
                    $dataseries = array();
                    foreach ($level as $key => $value) {
                        // $dataseries[] = array('label' => $value['label'], 'data' => round($value['nb']));
                        $dataseries[] = array($value['label'], round($value['nb']));
                    }
